  ...
  return (
    <div>
      <PageHead
        ...
        right={
          <button className="rounded-lg bg-gradient-to-r from-violet-500 to-cyan-500 px-4 py-2 text-sm font-semibold text-white">
            + Invite PM
          </button>
        }
      />
      ...
      <div className="glass divide-y divide-white/5">
        {members.map((m) => (
          <div key={m.name} className="flex items-center gap-4 px-5 py-4">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-violet-500/60 to-cyan-500/60 text-xs font-bold text-white">
              {m.initials}
            </div>
            <div className="w-44">
      ...
      <div className="mt-6 rounded-2xl border border-amber-400/20 bg-amber-500/5 p-5 text-sm leading-relaxed text-slate-300">
        <span className="font-semibold text-amber-300">Why this works:</span> onboarding is
