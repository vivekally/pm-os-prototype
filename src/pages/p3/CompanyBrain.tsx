...
export default function CompanyBrain() {
  return (
    <div>
      <PageHead
        title="Company Brain"
        sub="One shared memory for the PM org. Per-source permissions keep private notes private while team knowledge compounds."
      ...
      <div className="space-y-3">
        {sharedKnowledge.map((k) => (
          <div key={k.q} className="glass glass-hover px-5 py-4">
            <div className="text-sm font-semibold text-cyan-200">"{k.q}"</div>
      ...
      <div className="mt-6 text-xs text-slate-500">
        Answers respect permissions: a PM only sees what their sources allow.
      </div>
    </div>
  );
}
