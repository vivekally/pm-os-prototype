  ...
  return (
    <div>
      ...
      <div className="glass p-5">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-slate-200 outline-none"
          />
          <button
            onClick={() => setAsked(true)}
            className="rounded-xl bg-gradient-to-r from-violet-500 to-cyan-500 px-5 py-3 text-sm font-semibold text-white"
          >
            Ask
          </button>
        </div>
          ...
          <div className="mt-5 space-y-4">
            ...
            <div>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {cited.map((m) => (
                  <div key={m.id} className="rounded-xl border border-cyan-400/20 bg-cyan-500/5 p-4">
      ...
      <div className="mt-6 grid gap-3 text-sm text-slate-400 md:grid-cols-3">
        {[
          "What did we promise Acme in the last QBR?",
          "Which experiments touched pricing this year?",
          "Who owns the platform migration risk?",
        ].map((q) => (
