        ...
        {shown.map((s) => (
          <div key={s.id} className="glass glass-hover flex flex-col p-5">
            <div className="flex items-start justify-between gap-2">
              <code className="text-xs text-cyan-300">{s.command}</code>
        ...
        >
          ...
          >
            <div className="mb-3 flex items-center justify-between">
              <code className="text-sm text-cyan-300">{open.command}/SKILL.md</code>
              <button onClick={() => setOpen(null)} className="text-slate-400 hover:text-white">
                ✕
              </button>
            </div>
            <pre className="whitespace-pre-wrap rounded-xl bg-black/40 p-4 text-xs leading-relaxed text-slate-300">
              {open.skillMd}
            </pre>
